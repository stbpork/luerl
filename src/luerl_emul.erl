...
set_userdata(#uref{i=N}, Udata, #luerl{utab=Us0}=St) ->
...
gc(#luerl{ttab=Tt0,tfree=Tf0,ftab=Ft0,ffree=Ff0,utab=Ut0,ufree=Uf0,
    ...
    GcT = #gct{t=Tt0,s=[]},                     %Tables
    GcF = #gct{t=Ft0,s=[]},                     %Frames
    GcU = #gct{t=Ut0,s=[]},                     %Userdata
...
mark([#uref{i=U}|Todo], More, GcT, GcF, #gct{s=Seen0}=GcU) ->
    case ordsets:is_element(U, Seen0) of
	true ->					%Already done
	    mark(Todo, More, GcT, GcF, GcU);
	false ->
	    Seen1 = ordsets:add_element(U, Seen0),
	    mark(Todo, More, GcT, GcF, GcU#gct{s=Seen1})
    end;
mark([#function{env=Env}|Todo], More, GcT, GcF, GcU) ->
    mark(Todo, [Env|More], GcT, GcF, GcU);
%% Catch these as they would match table key-value pair.
mark([{function,_}|Todo], More, GcT, GcF, GcU) ->
    mark(Todo, More, GcT, GcF, GcU);
